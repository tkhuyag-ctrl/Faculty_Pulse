"""
Bulk fetch ALL faculty publications, awards/grants, and talks from OpenAlex
Includes: Publications, Grants/Fellowships (as Awards), and Conference Talks
All data from 2020+
"""
import sys
import json
...
# Setup logging
log_file = f'bulk_faculty_complete_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
logging.basicConfig(
    level=logging.INFO,
...
def process_faculty(faculty_info: dict, chroma: ChromaDBManager) -> dict:
    """Process one faculty member - publications, awards, and talks"""
    ...
    try:
        ...
        if publications:
            ...
            # Extract grants/awards from publications
            ...
            for pub in publications:
                try:
                    ...
                    if source == 'pdf':
                        result['pub_pdf_count'] += 1
                    elif source == 'abstract':
                        result['pub_abstract_count'] += 1
        ...
        logger.info(f"    Publications: {result['publications_added']} ({result['pub_pdf_count']} PDF, {result['pub_abstract_count']} abstract)")
...
def main():
    """Main execution"""
    print("\n" + "="*80)
    print("BULK FACULTY FETCHER: PUBLICATIONS + AWARDS + TALKS")
    print("="*80 + "\n")
    ...
    print("  - Publications (2020+) with full PDF text when available")
    ...
    total_pubs_found = sum(r['publications_found'] for r in results)
    total_pubs_added = sum(r['publications_added'] for r in results)
    total_pub_pdf = sum(r['pub_pdf_count'] for r in results)
    total_pub_abstract = sum(r['pub_abstract_count'] for r in results)
    ...
    print(f"\n📚 PUBLICATIONS:")
    print(f"  Found: {total_pubs_found}")
    print(f"  Added: {total_pubs_added}")
    print(f"    - With full PDF text: {total_pub_pdf} ({total_pub_pdf/total_pubs_added*100 if total_pubs_added > 0 else 0:.1f}%)")
    print(f"    - With abstract only: {total_pub_abstract} ({total_pub_abstract/total_pubs_added*100 if total_pubs_added > 0 else 0:.1f}%)")
    ...
    print(f"\n📊 TOTAL DOCUMENTS ADDED: {total_pubs_added + total_awards_added + total_talks_added}")

    # Save results
    results_file = f"bulk_complete_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
