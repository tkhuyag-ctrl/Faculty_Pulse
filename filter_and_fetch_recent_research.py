...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('filter_research.log', encoding='utf-8')
    ]
)
...
def main():
    ...
    # Filter: Keep only faculty with OpenAlex IDs
    faculty_with_openalex = [f for f in faculty_list if f.get('openalex_id')]

    print(f"\nFiltered to {len(faculty_with_openalex)} faculty with OpenAlex IDs")
    print(f"Removed {len(faculty_list) - len(faculty_with_openalex)} faculty without OpenAlex IDs")
    ...
    for i, faculty in enumerate(faculty_with_openalex, 1):
        name = faculty['name']
        ...
        try:
            print(f"[{i}/{len(faculty_with_openalex)}] {name} ({dept})...", end=" ", flush=True)
        except UnicodeEncodeError:
            print(f"[{i}/{len(faculty_with_openalex)}] {safe_name} ({safe_dept})...", end=" ", flush=True)
    ...
    output_file = "haverford_faculty_filtered_2020plus.json"

    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(faculty_with_openalex, f, indent=2, ensure_ascii=False)

    print("\n" + "="*80)
    print("SUMMARY")
    print("="*80)
    print(f"Original faculty count: {len(faculty_list)}")
    print(f"Filtered faculty (with OpenAlex ID): {len(faculty_with_openalex)}")
    print(f"Removed (no OpenAlex ID): {len(faculty_list) - len(faculty_with_openalex)}")
    print()
    ...
    if total_publications > 0:
        ...
        sorted_faculty = sorted(faculty_with_openalex,
