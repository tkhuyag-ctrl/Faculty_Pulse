"""
Fetch Laura Been's publications from OpenAlex with FULL PDF TEXT extraction
Downloads PDFs and extracts full text to add to database
"""
import sys
import requests
import time
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'laura_been_pdfs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8')
    ]
)
...
def search_faculty_openalex(name: str) -> str:
    """Search for faculty OpenAlex ID"""
    ...
    try:
        ...
        for i, author in enumerate(results, 1):
            ...
            logger.info(f"  {i}. {author_name} ({author_id}) - {institution}")

            if 'haverford' in institution.lower():
                logger.info(f"  ✓ Found Haverford match")
                return author_id
...
def main():
    """Main execution"""
    print("\n" + "="*80)
    print("LAURA BEEN PUBLICATION FETCHER WITH FULL PDF TEXT")
    print("="*80 + "\n")
    ...
    # Step 3: Download PDFs and add to database
    print("Step 3: Downloading PDFs and extracting full text...")
